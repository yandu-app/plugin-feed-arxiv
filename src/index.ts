...
class ArxivFeedAdapter implements FeedAdapter {
  id = 'org.arxiv';
  name = 'arXiv';
  ...
  constructor(private config: ConfigGetter) {}

  async fetch(options: FetchOptions): Promise<FetchResult> {
    const { categories, searchQuery } = options.config as {
      categories: string[];
      searchQuery?: string;
    };
    const limit = options.limit ?? (this.config.get<number>('feeds.adapters.arxiv.limit') ?? 50);
  ...
  private parseAtom(xml: string): FeedEntry[] {
